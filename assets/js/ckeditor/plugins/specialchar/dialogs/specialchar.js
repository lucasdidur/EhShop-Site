...
CKEDITOR.dialog.add( 'specialchar', function( editor ) {
	...
	var onFocus = function( evt, target ) {
			...
			if (target.getName() == 'a' && ( value = target.getChild(0).getHtml() ))
			{
				...
				var desc = target.$.title;

				dialog.getContentElement( 'info', 'charPreview' ).getElement().setHtml(value);
				htmlPreview.setHtml( CKEDITOR.tools.htmlEncode(desc) );
				...
				for (var j = 0; j < columns; j++, i++)
				{
					if ((character = chars[ i ]))
					{
						...
						else
						{
							...
							charDesc = (lang[_tmpName] ? lang[_tmpName] : ""); // || character;
						}
					...
					children: [ {
						...
						children: [
							...
							},
							{
								...
								style: 'border:1px solid #eeeeee;font-size:42px;height:60px;width:70px;padding-top:9px;text-align:center;',
								html: '<div>&nbsp;</div>'
							},
							{
								type: 'html',
