<?php
    ...
    class MinecraftColors
    {
        ...
        static private $formatting = array(
            'k' => 'padding:0 8px 0 4px; text-shadow:6px 2px 0px',      //Obfuscated
        ...
        static public function convertToMinecraft($html)
        {
            $html = str_replace('&nbsp;', ' ', $html);
            $html = html_entity_decode($html);

            $html = str_replace(self::LINE_BREAK, "\n", $html);
            $html = str_replace(self::CLOSE_TAG, "&r", $html);

            foreach (self::$colors as $index => $color)
            {
                $html = str_replace(sprintf(self::START_TAG, self::CSS_COLOR.$color), "&" . $index, $html);
            }

            foreach (self::$formatting as $index => $color)
            {
                $html = str_replace(sprintf(self::START_TAG, $color), "&" . $index, $html);
            }
